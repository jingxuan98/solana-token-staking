...
declare_id!("Ft9XAg4Uhqg36yydXY54UurH6KQPyqtLhQnTPYPoLun7");

fn mint_reward<'info>(
    reward: u64,
    staking_token: &InterfaceAccount<'info, Mint>,
    user_staking_wallet: &InterfaceAccount<'info, TokenAccount>,
    admin: &AccountInfo<'info>,
    token_program: &Interface<'info, TokenInterface>,
) -> Result<()> {
    let cpi_accounts = MintTo {
        mint: staking_token.to_account_info(),
        to: user_staking_wallet.to_account_info(),
...
pub mod solana_staking {
    ...
    pub fn stake(ctx: Context<Stake>, amount: u64) -> Result<()> { 
        ...
        // require!(ctx.accounts.staking_token.key() == pool_info.token, ErrorCode::InvalidToken);
        ...
        if user_info.amount > 0 {
            ...
            mint_reward(
                ...
                &ctx.accounts.user_staking_wallet,
                &ctx.accounts.admin,
    ...
    pub fn unstake(ctx: Context<UnstakeNClaim>) -> Result<()> {
    ...
    pub fn claim_reward(ctx: Context<UnstakeNClaim>) -> Result<()> {
...
pub struct Stake<'info> {
    ...
    // #[account(
    //     mut,
    //     constraint = staking_token.key() == pool_info.token @ ErrorCode::InvalidToken
    // )]
    pub staking_token: InterfaceAccount<'info, Mint>,
